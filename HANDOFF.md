...
### M10 — agreed 2026-09-21: project boundary, hardening, plumbing, product separation
Work in this order; each item is one or more commits with the gate green and a push.
1. **Project as the hard boundary.** Push project scoping into the data layer for every read path (builds, artifacts and downloads, execution-log `previous_with_log/1` and diffs, dashboards/tests pages, Prometheus `/metrics` per project or admin only). Per-project retention days and blob key prefix. A router-walking test: for every route, a user allowed on project A gets 404 on project B's ids and slugs.
2. **Fragile parts found by the trial.** (a) No stray lifecycle workers: `invocation_attempt_started` on a node without a stream worker must create/touch the row directly, never start a worker; keep the takeover path as defence. (b) Chaos soak: `mix conveyor.loadgen --hosts a,b --drop-after N --retries 5 --verify` for hours against two nodes behind the NLB plus `kill -9`s; pass = zero `in_progress` leftovers, zero `{:fenced}` on lifecycle RPCs, oracle green. (c) Root-cause the Oban stall (24 `ParseExecLog` jobs stayed `available` on the trial server even after `Oban.retry_all_jobs`; queue unpaused, empty, notified) and add an alert rule "available jobs older than 10 min".
3. **Plumbing.** Opt-in `DATABASE_SSL=true` with `DATABASE_SSL_CA` (bundle path; AWS RDS global bundle documented) so `rds.force_ssl` stays on; a Caddy sidecar option for single-node deployments (Let's Encrypt for 443 and h2 for 1985; drop the NLB in `deploy/trial` for the staging shape); execution-log parser: intern paths/digests per log and cap `input_files` per spawn; one rehearsed backup+restore (RDS snapshot + blob bucket into a fresh stack); Prometheus alert rules for ack latency, fenced commits, parse backlog, blob errors.
4. **Product separation in the UI.** Project home page (keys, retention, cache endpoint, segments, audit for that project), per-project admin groups instead of the global admin, compare/previous-build confined to project and branch.
