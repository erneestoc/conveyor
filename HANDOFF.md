...
2. Bottleneck found (PLAN §21 M7): group commit time in `Conveyor.Ingest.Writer` (≈175 events per commit, median 250–500 ms, Postgres idle). Reduce statements per commit: batch all `event_segments`/`log_segments`/`targets`/`test_results`/`actions` rows of a group into one `insert_all` per table (keep the per-invocation fenced `UPDATE`), move zstd compression out of the transaction (already done in `Batch` at build time? verify), grow groups (`INGEST_BATCH_MAX_EVENTS`, `INGEST_BATCH_FLUSH_MS`, `INGEST_WRITER_FLUSH_MS`), and confirm with `:eprof` on one shard. Target: p99 < 250 ms at 200 streams on the laptop before reference-hardware runs.
