...
**A. Finish the dashboard (started).**
1. Stat tiles show the delta versus the previous period of the same length (builds, success rate, p50/p90/p99, cache hit rate): extend `Conveyor.Metrics.Dashboard` with a `previous_range/2` computation and render `▲/▼ x%` under each tile; golden test with a fixed seed (see D1).
2. Saved segments: a query string saved per project with a name (`segments` table: project_id, name, query, position; managed in Settings), shown as chips on the dashboard; "compare" renders two segments side by side (two `Scope`s, same panels). Default segments stay Local/CI.
3. Phase-over-time chart from `invocation_metrics.profile_summary["action_phases"]`: stacked area per day (cache check / upload / queued / remote execution / download / local / setup / outputs), summed across builds in the range; reuse `Charts.stacked_bars` with the phase palette in `ConveyorWeb.Timeline.phase_color/1`.
4. Cache hit rate by mnemonic (from `runner_counts`/actions or from profile summaries) and a "top cache-missing targets" table; regression list = targets whose p50 duration in the range rose more than 20 % versus the previous period (needs a per-target duration aggregate; `targets` rows carry `first_seen_at`/`completed_at`); queue-time trend (phase "queued" per day) as the RBE capacity signal; time-of-day heatmap of build starts.
5. Grooming: one palette and legend component for all charts, empty and loading states, `Charts.stacked_bars` hover already has titles, `Charts.lines` now has hover dots.

**B. Browser tests for the settings forms (started, `e2e/tests/settings.spec.mjs`).** The auth flow and the section rendering pass. Drive the forms next: inspect the rendered field names in Playwright's `error-context.md` (they come from form structs, not the template), then create a project, create a key (assert `#new-key-plaintext` shows `conveyor_`), rotate and revoke (buttons `phx-click=rotate_key|revoke_key`, confirm dialogs), put a cache endpoint with each TLS mode, and assert the audit log lists the actions. Run locally with `ADMIN_TOKEN=e2e-token` on both the server and the test; CI sets it in `browser.yml`.

**C. Explain the rebuild (the milestone's core).**
1. Ingest Bazel's compact execution log: client flag `--execution_log_compact_file=...` uploaded through the artifact API (`PUT /invocations/:id/artifacts/execution.log`, `tools/bes-upload-profile` gains a generic upload mode) or referenced through the CAS sink. Parse the zstd-framed protobuf (`spawn.proto` `ExecLogEntry`, vendor it under `priv/protos/`): entries are inputs (path, digest), spawns (target, mnemonic, input ids, output digests, cache hit, runner, timing).
2. Store: `action_inputs` (invocation_id, action key = target+mnemonic+primary output, input_path, digest) written as one row per (path, digest) with digests interned in an `input_digests` table to keep it small; `spawns` (invocation_id, action key, cache_hit, runner, digests of outputs, wall/queue/exec times).
3. Diff: for an action in build B, find the same action key in the previous build of the same project (same branch tag when present) and report inputs added, removed and changed by digest. Actions tab: "rebuilt because N inputs changed" with the paths; overview: count of rebuilt actions with a link.
4. Non-hermetic report (dashboard): actions whose inputs are identical to a previous run but which executed again or produced different output digests; grouped by target and mnemonic.
5. Also derive from the log: bytes uploaded and downloaded per build, cache hit per target over time (feeds A4).

**D. Make the numbers and the guarantees provable.**
1. Golden-data tests: a fixed seed (`Conveyor.Seed.replay/4` with `seed:`) plus hand-computed expected values for success rate, percentiles and cache hit per segment, asserted exactly in `Conveyor.Metrics.Dashboard` tests.
2. Two-node test in ExUnit: start a second `Conveyor.Ingest.Supervisor` (separate registry names) against the same database, stream a build to node A, then resume it on node B while A's worker is alive; assert A gets fenced, B completes, the oracle passes.
3. Property tests (StreamData is not a dependency yet): normalizer counters and offsets for random event sequences; writer merge of overlapping target rows across batches equals sequential application.
4. On-demand contract tests: `mix test --only s3` against a real bucket (env `S3_TEST_BUCKET`), `--only oidc` against Keycloak (`chumti-keycloak` runs locally) and later Okta/Google.
5. Mutation spot checks documented as a checklist in `docs/development.md`: break the fenced update, the ack ordering, the scrubber, the dedup path, and name the test that must fail for each.
