...
### M9 — proposed (PLAN §22): explain rebuilds from input digests, dashboard v2, browser tests for settings/auth
Started 2026-09-19: hover value dots on the line charts (`Charts.lines`), `ADMIN_TOKEN` read in base config so the admin sign-in is testable in dev/CI, browser tests for the admin-token flow and the settings sections (`e2e/tests/settings.spec.mjs`, run with `ADMIN_TOKEN=e2e-token` on both the server and the test). Not yet: driving the create-project / create-key / rotate / revoke / cache-endpoint forms in the browser (inspect the rendered field names first, Playwright's error-context snapshot shows them); period-over-period deltas on the stat tiles; the rest of PLAN §22 (execution-log ingest and the rebuild diff, dashboard v2 charts).
