...
2. Bottleneck found (PLAN §21 M7): group commit time in `Conveyor.Ingest.Writer` (≈175 events per commit, median 250–500 ms, Postgres idle). Segments are already grouped per commit (commit a51e272; small gain). Profile first: `:eprof` on one writer shard (`Conveyor.Ingest.Writer.name(0)`) during a 200-stream run to split DB round trips vs BEAM work, then group the remaining upserts (targets/tests/actions/tag_keys across the batches of a group; conflict targets unchanged; keep the per-invocation fenced `UPDATE`), grow groups (`INGEST_BATCH_MAX_EVENTS`, `INGEST_BATCH_FLUSH_MS`, `INGEST_WRITER_FLUSH_MS`), and confirm with `:eprof` on one shard. Target: p99 < 250 ms at 200 streams on the laptop before reference-hardware runs.
