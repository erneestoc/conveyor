...
3. **CAS sink in v1** (M5) or defer to post-v1 and rely on external cache + upload API.
...
- **M5 (in progress).** Tier A timeline tab shipped: phases from timing metrics, one lane per test attempt and per reported action, SVG, live-updating, capped at 400 rows. Remaining: blob store, bytestream fetcher with per-project cache credentials, HTTP upload API, optional CAS sink, Tier B profile timeline (worker parse + canvas), profile summary job, test.log/test.xml viewing.
- **M6 (part 1 done).** Settings page (`/settings`): projects (create/archive), API keys per project (create shown once with a ready-to-paste bazelrc line, default tags, rotate with grace, revoke, expiring-key banner). Reachable by everyone until OIDC lands. Remaining M6: OIDC login, roles/admin gating, audit log, per-key limits, CSP/headers, threat model.
